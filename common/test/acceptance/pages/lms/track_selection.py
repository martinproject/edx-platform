...
class TrackSelectionPage(PageObject):
    ...
    def __init__(self, browser, course_id, separate_verified=False):
        ...
        if separate_verified:
            self._separate_verified = "?separate-verified=1"
        else:
            self._separate_verified = "?disable-separate-verified=1"

    @property
    def url(self):
        """Return the URL corresponding to the track selection page."""
        url = "{base}/course_modes/choose/{course_id}{separate_verified}".format(
            base=BASE_URL,
            course_id=self._course_id,
            separate_verified = self._separate_verified
        )
